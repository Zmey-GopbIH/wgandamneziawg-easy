# Генерация bcrypt-хэшированного имени пользователя и пароля

В файле [`docker-compose.yml`](docker-compose.yml), вместо строки имени пользователя и пароля в виде простого текста, требуется имя пользователя и пароль, хэшированный с помощью bcrypt. В этом документе объясняется, как сгенерировать хэш на основе пароля в виде простого текста (хеш имени пользователя генерируется аналогично).
...
- Вы используете [`docker-compose.yml`](docker-compose.yml)
   ...
   Самый простой способ сгенерировать хэш пароля bcrypt с помощью wgpw — использовать docker и node:
    ...
    ```sh
    docker run ghcr.io/nonenulldev404/wgandamneziawg-easy:latest node -e 'const bcrypt = require("bcryptjs"); const hash = bcrypt.hashSync("Укажите_Ваш_пароль_или_Имя_пользователя", 10); console.log(hash.replace(/\$/g, "$$$$"));'
    ```
...
- Вы используете `docker run`
    ...
    Если Вы используете `docker run` для запуска wgandamneziawg-easy, вы должны заключить строку хэша в одинарные кавычки ( '...'). Вы можете использовать эту команду:

    ```sh
    docker run --rm ghcr.io/nonenulldev404/wgandamneziawg-easy:latest node -e "const bcrypt = require('bcryptjs'); const hash = bcrypt.hashSync('Укажите_Ваш_пароль_или_Имя_пользователя', 10); console.log('\'' + hash + '\'');"
    ```
...
```sh
docker run ghcr.io/nonenulldev404/wgandamneziawg-easy:latest wgpw Укажите_Ваш_пароль_или_Имя_пользователя
```
